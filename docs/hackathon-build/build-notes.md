...
- Hosted verification: added an explicit-opt-in HTTPS verifier for invite-only Auth, same-household sharing, foreign-household Data/RPC/Storage rejection, positive Realtime delivery, and foreign Realtime non-delivery. It rejects loopback, service-role/secret keys, and missing inputs. Current result is `SKIP / non-connected` because no HomeRelay cloud project or invited accounts were supplied.
...
- Remaining gates: this proves local Supabase and separate authenticated browser contexts, not a hosted project or two physical devices. HTTPS smartphone camera/microphone permission, a separate physical PC, cloud Supabase, live Qdrant/Neo4j/Datadog, CodeRabbit PR review, and public GitHub push remain explicitly unverified.
