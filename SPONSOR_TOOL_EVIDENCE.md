...
|---|---|---|---|---|---|---|
| OpenAI Codex | **使用済み** | 企画資料・要件の照合、実装、テスト、セキュリティ監査、修正記録 | リポジトリ全体、`docs/hackathon-build/checklist.md`、`build-notes.md`、Git checkpoint | CodeRabbitの最新9件をcommit `76500b9…`で検証・修正し、最終増分reviewもactionable 0件で完了。lint、typecheck、全unit 580/580、synthetic E2E 16/16、production build、215公開候補／43 browser配信ファイルのprivacy/secret監査、dependency audit 0を確認。先行してSupabase RLS/Storage/Realtimeのcloud受入も実行 | 主導線と、checklist/build-notesの検証記録 | このCodex作業で使用。リポジトリ内API key不要 |
...
| CodeRabbit | **実レビュー済み・使用済み / 最終増分actionable 0件** | PUBLIC GitHub PRの実レビュー、認証・privacy・accessibility・fallback重点確認 | `.coderabbit.yaml`、`.github/pull_request_template.md`、PR #1のreview／修正差分 | `rin0908/homerelay-web-fast-hacks-2026-public`だけに接続。初回full reviewのactionable 23件を分類・対応し、続くactionable 4件＋outside-diff 1件、9件を修正。commit `76500b9…`の25ファイル増分reviewは`success`、actionable 0件。Docstring Coverage warning 1件（0% / 80%）はactionableとは別のスタイル指標として受容 | [PR #1](https://github.com/rin0908/homerelay-web-fast-hacks-2026-public/pull/1)、[初回review](https://github.com/rin0908/homerelay-web-fast-hacks-2026-public/pull/1#pullrequestreview-5086311772)、[1回目の増分review](https://github.com/rin0908/homerelay-web-fast-hacks-2026-public/pull/1#pullrequestreview-5088079735)、[2回目の増分review](https://github.com/rin0908/homerelay-web-fast-hacks-2026-public/pull/1#pullrequestreview-5090633444)、[最終follow-up](https://github.com/rin0908/homerelay-web-fast-hacks-2026-public/pull/1#issuecomment-5505274202) | GitHub App以外の秘密値不要。PRはOPEN・未merge |
...
- 上記60秒／56秒とvendor read-backは`f996dc6…`より前のPRIVATE Previewの証拠です。CodeRabbit最新9件への修正commit `76500b9…`はローカルrelease検証とactionable 0件の最終増分reviewに合格していますが、Preview再deployと物理2端末再試験は未実施です。
...
4. CodeRabbit: **実レビュー済み・使用済み**。PUBLIC PR #1で初回actionable 23件を取得・分類し、次のactionable 4件＋outside-diff 1件、さらに9件を修正。`76500b9…`の25ファイル増分reviewは`success`、actionable 0件で完了。Docstring Coverage warning 1件（0% / 80%）はactionableと分けて記録した。
