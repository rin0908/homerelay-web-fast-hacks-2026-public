...
|---|---|---|---|---|---|---|
| OpenAI Codex | **使用済み** | 企画資料・要件の照合、実装、テスト、セキュリティ監査、修正記録 | リポジトリ全体、`docs/hackathon-build/checklist.md`、`build-notes.md`、Git checkpoint | CodeRabbitの全指摘を検証し、最新の認証／action回復2件をcommit `a4bb941…`で修正。最新8ファイル増分reviewはactionable 0件。lint、typecheck、全unit 590/590、synthetic E2E 16件、production build、privacy/secret監査を確認。Supabase／Qdrant／Neo4jの対象限定cleanupも安全監査して実行 | 主導線と、checklist/build-notesの検証記録 | このCodex作業で使用。リポジトリ内API key不要 |
...
| Qdrant | **実接続済み・live検証PASS** | 類似する過去の申し送り、未購入品の重複候補 | `lib/qdrant/`、`scripts/bootstrap-qdrant.mjs`、`scripts/verify-qdrant.mjs`、`app/api/entries/[entryId]/related/route.ts`、`components/RelatedHandoffsPanel.tsx` | Cloud Freeでbootstrap／Inference／世帯filterがPASS。最新物理試験でも候補を表示し、別世帯0件。最終cleanup前はledger scopeのfixture point 1、単一cleanup後の即時／2秒後read-backはいずれも0 | liveの関連申し送り／必要品候補と別世帯非混入。明示demo modeでは引き続き`合成候補` | server-only設定で実接続済み。endpoint／keyの実値はこの文書へ記録しない |
| Neo4j | **実接続済み・live検証PASS** | 世帯、家族、親族、ヘルパー、申し送り、対応担当、購入担当の関係投影 | `lib/neo4j/`、`app/api/actions/route.ts`、`scripts/neo4j-schema.mjs`、`scripts/bootstrap-neo4j.mjs`、`scripts/verify-neo4j.mjs`、`docs/integrations/neo4j.md` | AuraDB Freeで5 constraintsとparameterized write/read-backがPASS。別世帯関係0件。最終cleanup前はledger scopeのfixture node 9／relationship 24、単一cleanup後の即時／2秒後read-backはいずれも0 | `ASSIGNED_TO`／`PURCHASE_ASSIGNEE`を含む関係read-backとforeign非混入 | server-only設定で実接続済み。URI／username／password／instance identifierの実値はこの文書へ記録しない |
...
- 2026-09-03にはPreview source commit `23138b0…`で実iPhone helperと別Windows familyを再試験し、1回目60秒以内（正確値未記録）、2回目42秒で2回連続成功しました。runtime read-backはdraft 200、confirmed entry 201、actions 5件すべて204、関連候補200、runtime error 0でした。
- fail-closed認証／action回復修正commit `a4bb941…`は別Previewへ配備し、READY、build成功、runtime error 0、`/login`入口200、390px viewportを確認しました。物理60秒以内／42秒は`23138b0…`へ帰属し、`a4bb941…`へ誤って帰属しません。
- 最終fixture cleanupは1回だけ実行しました。cleanup前はSupabase Auth 3／5表合計15行／Storage 1、Qdrant point 1、Neo4j node 9／relationship 24でした。即時と2秒後のread-backはいずれも全対象0件で、3つの一時合成パスワード、QR、ledger、配備用一時ファイルも削除済みです。
